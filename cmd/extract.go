...
func Extract(file, outputFile string) ([]string, error) {
	...
	for _, block := range blocks {
		...
		case markdown.CommentaryBlock:
			commands = append(commands, fmt.Sprintf("showboat build %s commentary %s", quotedTarget, shellQuote(b.Text)))
		case markdown.CodeBlock:
			if b.IsImage {
				commands = append(commands, fmt.Sprintf("showboat build %s image %s", quotedTarget, shellQuote(b.Code)))
			} else {
				commands = append(commands, fmt.Sprintf("showboat build %s run %s %s", quotedTarget, b.Lang, shellQuote(b.Code)))
			}
