	...
	for i, block := range blocks {
		...
		case markdown.CodeBlock:
			if b.IsImage {
				...
				cmd := fmt.Sprintf("showboat exec %s %s", quotedTarget, b.Lang)
				// Check if the following output block has a language set.
				if i+1 < len(blocks) {
					if ob, ok := blocks[i+1].(markdown.OutputBlock); ok && ob.Lang != "" {
						cmd += " --output-lang " + ob.Lang
					}
				}
				cmd += " " + shellQuote(b.Code)
